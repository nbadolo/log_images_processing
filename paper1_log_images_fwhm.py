...
et profile radial d'intensité. Code okay à la date du 05 dec 2023. Test pour intégrer le calcul de la largeur à mi hauteur!!!
"""
...
from AymardPack import calculate2_fwhm as cal_fwhm
import webbrowser
    ...
    if n_lst_fltr3 == 0: # si l'étoile et sa psf n'ont pas de data en commun pour le même filtre
           ...
           if n_lst_fltr2 == 0 :
               ...
               for l in range(n_lst_fltr2) :
                   ...
                   mean_sub_v_arr = np.empty((n_fsize,nFrames,nSubDim//2-1))
                   #mean_sub_v_arr3 = np.empty((n_fsize,nFrames,nSubDim//2-1))
                   ...
                   for z in range(n_fsize) :
                        ...
                        for i in range (nFrames):
                             ...
                             mean_sub_v_arr[z][i] = mean_sub_v2 
                             sub_v_arr[z][i] = sub_v
                        ...
                        im1 = ax1.imshow(sub_v_arr[z][0], cmap='inferno', origin='lower',
                                      vmin= np.min(sub_v_arr[z][0]), vmax= np.max(sub_v_arr[z][0]), extent = [x_min , x_max, y_min , y_max])
                        #print('vmin =' + str(np.min(sub_v_arr[z][0])))  
                        #print('vmax =' + str(np.max(sub_v_arr[z][0]))) 
                        plt.text(-0.65*pix2mas*size[0]//2., 2*pix2mas*size[1]//5.,
                                                f'{star_name_im}', color='w',
                                            fontsize=label_size3_, ha='center', fontweight ='bold')#, fontstyle ='italic' )
                        
                        plt.text(-0.8*pix2mas*size[0]//2., -2.3*pix2mas*size[1]//5.,
                                                f'{fltr_arr[l][z]}', color='w',
                                            fontsize=label_size3_, ha='left', fontweight ='bold')#, fontstyle ='italic')
                        ...
                        if im1_max < 1e4 :
                            cb1 = fig.colorbar(im1, cax=cax, orientation='vertical',ticks=[1e3,2e3, 3e3, 4e3, 5e3], **cmapProp) # creation de la colobar
                        elif 1e4 < im1_max < 1e5 :
                            cb1 = fig.colorbar(im1, cax=cax, orientation='vertical',ticks=[1e4,2e4, 3e4, 4e4, 5e4], **cmapProp) # creation de la colobar
                        elif 1e5 < im1_max < 1e6 :
                            cb1 = fig.colorbar(im1, cax=cax, orientation='vertical',ticks=[1e5,2e5, 3e5, 4e5, 5e5], **cmapProp) # creation de la colobar
                        else :
                            cb1 = fig.colorbar(im1, cax=cax, orientation='vertical', **cmapProp) # creation de la colobar
                        cb1.ax.tick_params(labelsize = label_size_) # la taille de la graduation de la colobar
                        ...
                        cb1.ax.yaxis.get_offset_text().set(size=label_size_, weight = 'bold') #taille et style de l'expodant
                        for tick in cb1.ax.yaxis.get_major_ticks():  #for color bar tick in bold
                            tick.label2.set_fontweight('bold')
                        
                        
                        
                        ax1.tick_params(axis = 'both', labelsize = label_size_, width=5, length=10)
                        ax1.set_xticks([-150 , 0, 150])
                        ax1.set_yticks([-150, -75, 0, 75, 150])
                        ax1.set_yticklabels(ax1.get_yticks(), weight='bold')
                        ax1.set_xticklabels(ax1.get_xticks(), rotation = 0, weight='bold', size=label_size_)
                        # ax1.set_xlabel('Relative RA (mas)', fontsize = label_size2_,  weight='bold')
                        # ax1.set_ylabel('Relative Dec (mas)', fontsize = label_size2_, weight='bold')
                        
                        #pour le profile radial et le calcul de la fwhm
                        plt.subplot(212)
                        plt.plot(r_mas, mean_sub_v_arr[z][1], color='darkorange',
                        linewidth = 2, label= f'{star_name}') 
                        #plt.plot(r_mas, mean_sub_v_arr3[z][1],color='purple',
                        ...
                        plt.savefig('/home/nbadolo/Bureau/Aymard/Donnees_sph/large_log/'+star_name+
                                          '/plots/star_psf/'+ star_name +'_' +  f'{fltr_arr[l][z]}' + '_lin' + '.pdf', 
                                          dpi=100, bbox_inches ='tight')
                        
                        plt.savefig('/home/nbadolo/Bureau/Aymard/Donnees_sph/large_log/'+star_name+
                                          '/plots/star_psf/'+ star_name +'_' +  f'{fltr_arr[l][z]}'  + '_lin' + '.png', 
                                          dpi=100, bbox_inches ='tight')         
                        
                        plt.savefig('/home/nbadolo/Bureau/Aymard/Donnees_sph/All_plots/star_psf/paper1_fwhm/pdf/'+star_name+
                             '_' +  f'{fltr_arr[l][z]}' + '_lin' + '.pdf', dpi=100, bbox_inches ='tight')
                        
                        plt.savefig('/home/nbadolo/Bureau/Aymard/Donnees_sph/All_plots/star_psf/paper1_fwhm/png/'+star_name+
                                          '_' +  f'{fltr_arr[l][z]}' + '_lin' + '.png', dpi=100, bbox_inches ='tight')
        ...
        for l in range(n_lst_fltr3):
            ...
            delta_fwhm_arr = np.empty((n_fsize))
            mean_sub_v_arr2 = np.empty((n_fsize, nFrames2,nSubDim//2-1))
            mean_sub_v_arr3 = np.empty((n_fsize, nFrames3,nSubDim//2-1))
            ...
            for z in range(n_fsize) :
                ...
                for i in range (nFrames2):
                      ...
                      #fwhm_str = cal_fwhm(r_mas, mean_sub_v_arr2[z][0] )
                      fwhm_str = cal_fwhm(r_mas, mean_sub_v_arr2[z][0]/(np.max(mean_sub_v_arr2[z][0])) )
                      print('la fwhm de létoile est ' + str(fwhm_str))
                ...
                for j in range (nFrames3):
                      hdu3 = fits.open(file_lst3[j])[0]  
                      data3 = hdu3.data   
                      i_v3 = data3[z,:,:]
                      ...
                      mean_sub_v_arr3[z][j] = mean_sub_v3 
                      sub_v_arr3[z][j] = sub_v3
                      
                      #fwhm_psf = cal_fwhm(r_mas, mean_sub_v_arr3[z][0] )
                      fwhm_psf = cal_fwhm(r_mas, mean_sub_v_arr3[z][0]/(np.max(mean_sub_v_arr3[z][0]))) # le profile est normalisé
                      print('la fwhm de la psf est ' + str(fwhm_psf))
                      ...
                      #Calcul du ratio des fwhm
                      
                delta_fwhm = fwhm_str/fwhm_psf
                delta_fwhm_arr[z] = delta_fwhm
                print( 'ratio = ' + str(delta_fwhm))
                ...
                im1 = ax1.imshow(sub_v_arr2[z][0], cmap='inferno', origin='lower',
                              vmin= np.min(sub_v_arr2[z][0]), vmax= np.max(sub_v_arr2[z][0]), extent = [x_min , x_max, y_min , y_max])
                #print('vmin =' + str(np.min(sub_v_arr2[z][0])))  
                #print('vmax =' + str(np.max(sub_v_arr2[z][0]))) 
                ...
                cb1.formatter.set_powerlimits((0, 0)) # notation scientifiq de la colorbar ( exple : 1e5)
                #cb1.formatter.set_useMathText(True)  # notation scientifiq de la colorbar ( exple : x 10⁵)
                cb1.ax.yaxis.get_offset_text().set(size=label_size, weight = 'bold') #taille et style de l'expodant
                for tick in cb1.ax.yaxis.get_major_ticks():  #for color bar tick in bold
                    tick.label2.set_fontweight('bold')
                ...
                im2 = ax2.imshow(sub_v_arr3[z][0], cmap='inferno', origin='lower',
                              vmin= np.min(sub_v_arr3[z][0]), vmax= np.max(sub_v_arr3[z][0]), extent = [x_min , x_max, y_min , y_max])
                plt.text(-0.6*pix2mas*size[0]//2., 2*pix2mas*size[1]//5.,
                                        f'{psf_name}', color='w',
                        fontsize=label_size3, ha='center', fontweight ='bold')#, fontstyle ='italic' )
                ...
                if im2_max < 1e4 :
                    ...
                    cb2 = fig.colorbar(im2, cax=cax, orientation='vertical',ticks=[1e4,2e4, 3e4, 4e4, 5e4], **cmapProp) # creation de la colobar
                elif 1e5 < im2_max < 1e6 :
                    cb2 = fig.colorbar(im2, cax=cax, orientation='vertical',ticks=[1e5,2e5, 3e5, 4e5, 5e5], **cmapProp) # creation de la colobar
                else :
                ...
                cb2.ax.yaxis.get_offset_text().set(size=label_size, weight = 'bold') #taille et style de l'expodant
                ...
                #Pour le profile radial et le calcul de la fwhm
                plt.subplot(223)
                plt.plot(r_mas, mean_sub_v_arr2[z][0]/np.max(mean_sub_v_arr2[z][0]), color='darkorange',
                linewidth = 4, label= f'{star_name}') 
                plt.plot(r_mas, mean_sub_v_arr3[z][0]/np.max(mean_sub_v_arr3[z][0]),color='purple',
                    linewidth = 4, label = f'{star_name}' + '_psf') 
                plt.legend(loc=0) 
                plt.xlabel('r (mas)', size=10) 
        ...
        return(f'{star_name}', fwhm_str, fwhm_psf, delta_fwhm, obsmod, f'{fltr_arr[l][z]}')
        
#star=log_image('*L02_Pup', 'both')    
